...
日期：2026-09-17。来源：C07（指定对话）及 C08（学习并完善本项目的授权）。**本次交付是方案修订，不是生产功能实现或新增实机验收。** 技术细化列入推荐设计；C09已落实RC-02逐核删除并确认前端复用方向，其他改变已确认要求的RC提案未自动生效。
...
以下记录提案与当前决定。**C09已明确删除逐物理核心温度，RC-02已落实；REQ-012/114退役，编号与历史内容保留。RC-01/03/04仍待决定。** 实现时只按现行范围验收，不再把逐核映射当作发布门槛。前端复用方向已确认，具体布局见[07](07-native-ui.md)。
...
| RC-02 逐核范围（C09已落实） | REQ-012/114已删除；009/010/122/132同步 | 删除逐物理核心温度及core_id要求；保留CPU温度来源／热区列表，不预设未来恢复承诺 | 删除需求不参与验收；不生成Core 0…N温度网格；按sourceID区分和绘图 |
...
- P0：RC-02已落实；评审RC-01/03/04，解决OQ-04，补齐V2-01/02和最终权限约束。
...
当前已讨论的路线均有以下落点；“写入文档”不等于已实现或已完成全部验收。C09删除逐核已同步到需求、数据模型、UI、阶段计划和追踪矩阵。

| 路线 | 设计入口 | 当前状态与下一步 |
|---|---|---|
| 产品范围与指标定义 | [01](01-requirements.md)、[15](15-decisions-and-corrections.md)、[16](16-open-questions.md) | 逐核及core_id已删除；主指标、可选能力和首发范围仍见RC-01/03/04 |
| SMC／HID／SMART／电池来源 | [03](03-sensor-acquisition.md)、[14](14-feasibility-validation.md) | 有M4单机CLI局部证据；下一步V2-01/02语义与身份验证 |
| 注册表、缓存、调度和组件边界 | [02](02-architecture.md)、[08](08-component-design.md)、本文第5～6节 | 已形成推荐设计，尚未写成生产构件；待V2-03/04 |
| Raw、EMA、聚合、趋势、SQLite、72小时 | [04](04-data-storage.md)、[05](05-processing-pipeline.md) | 保留既有要求；待算法/存储细节冻结和V2-05；OQ-04教学路径仍待确认 |
| 能力缺失、重试、Fatal与背压 | [06](06-error-handling.md)、[08](08-component-design.md) | 现行规则保留；RC-03获确认前不改为可选项故障隔离 |
| 菜单栏、弹出面板、主窗口与图表 | [07](07-native-ui.md)、[09](09-technology-selection.md) | C09确认开源复用方向；已指定组件、布局、交互、许可与适配边界，尚未实现App |
| 自动化测试、实机、长期验证 | [10](10-test-strategy.md)、[14](14-feasibility-validation.md)、[17](17-traceability.md) | 原型局部检查已执行；UI、数据链、72小时及发布测试仍待实现后执行 |
| Git／Issue／PR／阶段门禁 | [11](11-git-github-workflow.md)、[12](12-development-plan.md) | 继续当前V0分支与Draft PR #4；正式独立功能另起分支，门禁完整后merge commit并保留分支 |
| 生命周期、运维、签名及分发 | [13](13-operations-distribution.md) | 已记录路线；签名身份、分发形态及正式配置实测待落实 |

UI适配应优先利用MacMonitor的分组结构和Stats来源列表，再接入本项目数据模型；上游CPU占用率CoreTile不用于温度。固定文件和哈希见[UI来源清单](research/2026-09-17-native-ui-sources.json)。
