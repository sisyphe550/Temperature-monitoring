...
- 2026-09-17 C09 已删除逐物理核心温度及其核心标识要求。REQ-012、REQ-114 保留为退役记录，编号不复用，不参与当前验收；本表134个登记编号中132项现行、2项已删除。
...
2026-09-17 增补：[开源复核方案第4节](19-reference-informed-design.md) 提供 RC-01～05。C09 已落实 RC-02 的逐核删除，且不保留未来版本自动恢复的承诺；RC-01 主指标改名、RC-03 可选指标故障隔离、RC-04 首发兼容范围仍待决定。前端借鉴开源项目的方向已确认，具体基线见 [07](07-native-ui.md)。
...
- 状态：已确认
- 来源：C01（v0.3 对应原条号）；C09删除逐核后同步范围
- 验收边界：原型已有局部证据，完整产品验收未执行；具体状态见追踪矩阵。
...
- 状态：已确认
- 来源：C01（v0.3 对应原条号）；C09删除逐核后同步范围
- 验收边界：不再建立物理Core温度类型；CPU来源与派生主指标分别标识，Package含义仍见OQ-01。C09删除逐核要求不等于批准RC-01主指标公式。
...
- 状态：已确认
- 来源：C01
- 验收边界：采用07的左键弹出/关闭、右键命令菜单、Esc或点击外部关闭方案；主窗口与采集生命周期分离。
...
- 状态：设计基线
- 来源：C01；C09授权借鉴开源前端
- 验收边界：以MacMonitor分组面板、Stats列表为参考，遵循07的原生材质和布局基线；尊重系统减少透明度等设置。
...
- 状态：已确认
- 来源：C01
- 验收边界：采用07的温度概览、温度来源列表和图表工作区；不包含物理核心温度卡片。
...
- 状态：已确认
- 来源：C01
- 验收边界：使用加工值，按温度来源绘制带°C单位的柱状图；不按物理核心数生成柱体，不复用CPU占用率百分比含义。
...
- 模式：Unwanted
- 正文：如果某温度来源的物理含义未得到验证，则传感器描述构件应将该来源的映射状态标为未确认。
- 状态：设计基线
- 来源：C03、C04；C09删除逐核范围后保留真实性约束
- 验收边界：未知来源进入明确标注的诊断详情，不伪标CPU物理核心；CPU域、Package和电池语义仍需验证。
