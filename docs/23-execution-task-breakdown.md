...
- SensorTransport原始事实必须经Registry资格化；ReadingOutcome、PersistenceLease/ProcessingReceipt和PresentationState是唯一公开状态边界。
- copied/modified代码必须登记固定commit、路径、许可hash、notice和修改说明；R04/R07只允许method-only。
...
2. `ProcessingEngine`只有在PersistenceLease经SessionPersistence commit并取得匹配ProcessingReceipt后才交换算法状态；由T05.1、T05.3、T05.5测试。
...
6. running/fatal、温度值状态和历史状态不得通过独立布尔值形成矛盾组合；由T07.1～T07.5测试。
7. 上游旧值回退、名称物理语义、0°C补值、root/helper/写SMC及未登记复制代码不得进入Release；由T02.5、T08.1、T08.3、T11.1测试。
...
| 任务 | 依赖 | 文件 | 设计与产出 | 先失败/验证命令 | 交接与commit |
|---|---|---|---|---|---|
| - [ ] T02.1 JSON线协议 | T01.4 | `SensorRuntime/{SensorTransport,WorkerProtocol}.swift`、`Tests/Fixtures/ProtocolWorker/main.swift`、`WorkerProtocolTests.swift` | 原始DiscoveredCatalog/TransportReadBatch；version/UUID requestID/generation/command严格枚举；1MiB上限；Int64不经Double | 覆盖成功、非法UUID、坏JSON、超长、错version/ID/generation、未知command；wire不含SourceID语义 | T02.2消费编码器/解析器；`feat: define bounded sensor transport protocol` |
| - [ ] T02.2 WorkerClient管道 | T02.1 | `WorkerClient.swift`、`WorkerLifecycleTests.swift` | WorkerClient只实现SensorTransport；绝对路径、单请求、持续排空stderr、EOF/退出映射、close幂等 | fixture场景`success/oversize/badJSON/wrongID/oldGeneration/crash`；原始结果不能直接构造ReadRequest | T02.5通过资格化后公开client；`feat: add sensor worker transport` |
| - [ ] T02.3 SMC桥接与解码 | T01.4 | `SensorBridge/{SensorBridge.c,include/SensorBridge.h}`、`SensorDecodingTests.swift`、third-party-v1、`THIRD_PARTY_NOTICES.md` | 只读open/keyinfo/read/close；ABI静态断言；flt小端、sp78大端有符号；复制/修改来源和许可逐文件登记 | 字节向量25.5°C、错长度、未知类型、NaN/Inf、大小写键；写SMC/root符号扫描为空 | T02.5取得可测试SMC provider；`feat: add audited read-only smc bridge` |
| - [ ] T02.4 NVMe/Battery/HID选择器 | T02.3 | `SensorBridge/{SensorBridge.c,include/SensorBridge.h}`、`TemperatureCore/Registry.swift`、`SensorDecodingTests.swift`、`SourceRegistryTests.swift` | 唯一Internal NVMe、SMART Kelvin；IOPS CFNumber非Bool优先；HID仅诊断且同名不合并 | SMART0/300K、父树循环/多设备、IOPS缺字段、TB优先级、HID缺事件；缺事件不得生成0°C | 输出能力原因而非假值；`feat: add optional sensor providers` |
| - [ ] T02.5 Registry与合格客户端 | T02.2/T02.4 | `Registry.swift`、`QualifiedSensorClient.swift`、`MetricResolver.swift`、`SourceRegistryTests.swift` | DiscoveredSource只含事实；Mac16,13精确12键/format；QualifiedSourceCatalog分available/unavailable；本代handle映射 | 12全/缺1/重复/类型改变、旧generation、未知handle、同名不同registryID、新身份、12来源不生成Core编号 | W04只取得合格定义和成员；`feat: qualify versioned sensor sources` |
| - [ ] T02.6 截止时间与回收 | T02.5 | `WorkerClient.swift`、`QualifiedSensorClient.swift`、worker入口、`WorkerLifecycleTests.swift` | read1s/discover10s；TERM250ms后KILL；确认退出才新generation；无法回收即Fatal | 永不回复、晚回复、TERM忽略、KILL后退出、无法确认退出、连续close；旧代不得形成ReadBatch | W05可安全调度合格client；`feat: bound sensor worker lifecycle` |

## W03：SQLite、查询与保留

| 任务 | 依赖 | 文件 | 设计与产出 | 先失败/验证命令 | 交接与commit |
|---|---|---|---|---|---|
| - [ ] T03.1 SQLite模块与会话初始化 | T01.4 | `CSQLite/{module.modulemap,shim.h}`、`Persistence/SessionPersistence.swift`、`SQLiteStore.swift`、schema资源、`SQLiteStoreTests.swift` | SessionPersistence实现SessionStoreCapability并内部创建连接；SessionMetadata/open；schema/quick_check失败Fatal | 临时库验证user_version、FK、views、坏schema和完整metadata；调用者不能取得裸连接 | T03.2取得内部事务连接；`feat: initialize session persistence` |
| - [ ] T03.2 幂等批次事务 | T03.1 | `SessionPersistence.swift`、`SQLiteStore.swift`、`SQLiteStoreTests.swift`、`StorageFaultTests.swift` | 内部canonical hash/BEGIN IMMEDIATE；sources→definitions→segments→数据→receipt；同ID异载荷Fatal | 通过test-only fixture重复提交、ack丢失、rollback、主键同值/异值、Gap单调结束；无公开append旁路 | T05 writer可安全重试原批次；`feat: add idempotent persistence batches` |
| - [ ] T03.3 历史查询和降采样 | T03.2 | `SessionPersistence.swift`、`HistoryQuery.swift`、`HistoryQueryTests.swift` | SessionStoreCapability.query；1h/24h/72h分层；≤8series/2000点；全范围分箱保留min/max；250ms取消 | 8640桶含单峰99、两segment、旧query取消、超限、无数据 | W05/W07取得HistoryResult；`feat: add bounded history queries` |
| - [ ] T03.4 TTL、checkpoint与容量 | T03.3 | `SessionPersistence.swift`、`Retention.swift`、`RetentionTests.swift`、`StorageFaultTests.swift` | SessionStoreCapability.prune；父层先提交；120s宽限；有效页软限/文件硬限；WAL单独限制 | cutoff±1ns、sleep>72h、freelist解除软限、WAL64MiB、磁盘满、清理落后 | W05扩展同一actor的能力视图；`feat: enforce storage retention and capacity` |

## W04：加工算法

| 任务 | 依赖 | 文件 | 设计与产出 | 先失败/验证命令 | 交接与commit |
|---|---|---|---|---|---|
| - [ ] T04.1 校验、身份和Ring Buffer | T01.4 | `Processing/RingBuffer.swift`、`MonitorEngine.swift`、`RingBufferTests.swift`、`ProcessingIdentityTests.swift` | 只消费合格ReadBatch和ReadingOutcome；每series Raw/EMA各8192且300s；request/sample/generation/elapsed幂等 | failure不生成样本；NaN/Inf/−274、重复值、同ID异值、非递增时间、33rd series、旧值回退反例 | T04.2取得有序Sample；`feat: add validated bounded sample buffers` |
...
| - [ ] T04.5 趋势与Gap状态 | T04.4 | `Trend.swift`、MonitorEngine Gap部分、`TrendTests.swift` | OLS用实际时间；3点/80%；Gap后不跨段；变更方法接收Lease并仅暴露ProcessingReceipt | rising/stable/falling/insufficient、六种Gap原因、commit拒绝状态不前移、200→1000正常换档 | W05/W07取得趋势/断线；`feat: add segmented trends and gaps` |

## W05：调度、队列与闭环

| 任务 | 依赖 | 文件 | 设计与产出 | 先失败/验证命令 | 交接与commit |
|---|---|---|---|---|---|
| - [ ] T05.1 SessionPersistence能力与Lease | T03.2/T04.1 | `Persistence/{SessionPersistence,BoundedQueue,StorageWriter}.swift`、`BackpressureTests.swift` | 同一actor提供reservation/commit/session-store窄能力；Lease绑定强类型owner/generation/行/字节且不可公开构造/编码；writer仅内部排空 | 伪造、双消费、取消后提交、错owner/generation、超容量、16384/32MiB、高低水位均固定失败 | ProcessingEngine可在receipt后换状态；`feat: add bounded session persistence` |
...
| - [ ] T05.3 重试与安全watermark编排 | T04.4/T05.1/T05.2 | `MonitorEngine.swift`、`WatermarkTests.swift`、`BackpressureTests.swift` | request/gap/watermark先取得对应Lease；IO后加工并commit；receipt匹配后同步换状态；旧generation丢弃 | 请求0.95→1.05s、commit/receipt拒绝状态不前移、CPU整批重试、迟到回复 | T05.4取得一致内存/持久化状态；`feat: coordinate watermarks and persistence` |
| - [ ] T05.4 Controller快照和历史路由 | T03.3/T04.5/T05.3 | `MonitorEngine.swift`、`ControllerFixture.swift`、`MonitorIntegrationTests.swift` | 通过SessionStoreCapability open/query；≤5Hz bufferingNewest快照；5min内存，其余SQLite；persistedThrough | fixture脚本启动/读/换档/历史/stop；取消旧历史；UI慢消费 | W07只消费MonitorController；`feat: expose monitor controller streams` |
| - [ ] T05.5 软件端到端闭环 | T03.4/T05.4 | `MonitorIntegrationTests.swift`、W05验证报告 | QualifiedSensorClient→12Raw→max→EMA→aggregate→SessionPersistence→SQLite→history；receipt与DB恰好一次 | W05四套测试＋全Core覆盖；核对DB/内存sampleID、snapshot generation、BatchID | W06获得稳定核心；`test: verify monitoring software data loop` |
...
| 任务 | 依赖 | 文件 | 设计与产出 | 先失败/验证命令 | 交接与commit |
|---|---|---|---|---|---|
| - [ ] T07.1 互斥PresentationModel与Xcode壳 | T06.4 | Xcode工程、`App/TemperatureMonitorApp.swift`、`App/Presentation/{PresentationModel,PresentationState}.swift`、UI测试 | MainActor唯一转换Snapshot/HistoryResult；running/fatal、五种值状态、三种图表状态互斥；Debug/Test fixture、Release拒绝 | 状态转换、generation倒退、Fatal后running、basic/gap/stale/fatal、snapshot≤5Hz | 后继视图只绑定PresentationState；`build: scaffold native presentation state` |
| - [ ] T07.2 菜单栏与Popover | T07.1 | `App/Presentation/{StatusItemController,TemperaturePopover}.swift`、UI测试 | 只消费PresentationState；AppKit status item/popover；左右键、Esc/外点关闭；340×640限高 | `status.temperature`、live/cached/stale/unavailable、Fatal互斥、浅深色 | 输出稳定菜单栏入口；`feat: add temperature status popover` |
| - [ ] T07.3 Dashboard、来源与设置 | T07.2 | `App/Presentation/{TemperatureDashboard,TemperatureSourceRow,SettingsView}.swift`、UI测试 | 只消费sections/value状态；主窗尺寸；分组/evidence；五档；关窗不停采；无告警/自启 | `cpu.period`、`source.list`、⌘W/⌘Q、各互斥值状态 | T07.4取得选中series/range状态；`feat: add temperature dashboard and settings` |
| - [ ] T07.4 历史图表 | T05.5/T07.3 | `App/Presentation/{HistoryChartModel,HistoryChartView}.swift`、`HistoryChartModelTests.swift`、UI测试 | HistoryResult唯一转换HistoryChartState；5min/1h/24h/72h；≤8/2000；min/max；segment/Gap断线 | loading/ready/failed不可并存、previous不冒充成功、峰值95、Gap、多series | 输出可验证历史视图；`feat: add segmented temperature history chart` |
| - [ ] T07.5 Fatal、辅助功能和UI门禁 | T06.4/T07.4 | `FatalView.swift`、UI测试、W07报告 | FatalView只消费FatalPresentationState；30s可见回执；报告/复制/退出；NSAlert兜底；键盘/透明度/窄屏 | `fatal.code`/`fatal.quit`、running控件不与Fatal并存、渲染失败、完整XCUITest | W08取得可装配UI和截图；`test: complete native ui acceptance` |
...
| - [x] T09.4 73小时耐久与组合登记 | T09.3 | endurance证据、`docs/13-operations-distribution.md`资格矩阵 | 连续≥73h越过72h TTL；资源/DB/WAL/log/队列；全部套件通过后原子登记qualified combination | **2026-09-23 维护者取消≥73h实机长跑**；可选 endurance 工具保留；`qualified_combinations` 门禁为 sources+schedules+lifecycle | W10只接收此SHA/报告；`test: qualify long-running target build` |
